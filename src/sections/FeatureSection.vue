...
const { t } = useI18n({ useScope: "global" }); // Use the i18n instance

let isOpen = ref(1);
let translatedFeatures = ref([]);

watchEffect(() => {
  updateTranslatedFeatures();
});

function updateTranslatedFeatures() {
  translatedFeatures.value = features.map((feature) => ({
    id: feature.id,
    name: t(`features.${feature.id - 1}.name`),
    details: {
      ...feature.details,
      title: t(`features.${feature.id - 1}.details.title`),
      description: t(`features.${feature.id - 1}.details.description`),
    },
  }));
}
</script>
...
<template>
  ...
  <div id="features" class="container mx-auto px-5">
    <section class="py-16">
      ...
      <div class="mt-16">
        <!-- Tab Content -->
        <template v-for="feature in translatedFeatures" :key="feature.id">
          <div
