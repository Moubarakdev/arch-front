    ...
    <section class="py-16">
      <div class="w-4/5 md:w-3/5 mx-auto">
        ...
        <p class="text-theme-grayish-blue text-center mt-7 font-theme-content text-lg">
          Lorem ipsum dolor sit amet consectetur, adipisicing elit. Veritatis odit illum cumque.
        </p>
      </div>

      <div class="mt-10">
        <ul class="flex flex-col items-center md:flex-row justify-center font-theme-heading">
          <li v-for="feature in  features " :key="feature.id" :class="isOpen === feature.id
            ...
            <a @click.prevent="isOpen = feature.id" href="#"
              :class="isOpen === feature.id ? ' border-theme-secondary' : ''" class="py-5 md:border-b-0 border-b-4">{{
                feature.name }}</a>
          </li>
        </ul>
      </div>
      ...
      <div class="mt-16">
        ...
        <template v-for=" feature  in  features " :key="feature.id">
          <div v-show="isOpen === feature.id" class="grid gap-32 lg:grid-cols-2 items-center">
            <div class="relative">
              <img class="z-10 w-full" :src="feature.image" />
              <div class="-z-10 bg-theme-primary h-52 w-96 sm:h-80 sm:w-full rounded-r-full absolute -left-56 -bottom-16">
              </div>
            </div>
            <div>
...
export default {
  ...
  created() {
    this.axios.get('http://localhost:8000/api/v1/features/')
      .then(response => {
